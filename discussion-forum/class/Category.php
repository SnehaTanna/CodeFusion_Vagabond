...
class Category {	
	...
	public $category_id; // Define category_id property
    public $description; // Define description property
    public $categoryName;
	public $id;
	private $conn;
	...
	public function getCategory(){
		if($this->category_id) {
			$sqlQuery = "SELECT name FROM ".$this->categoryTable." WHERE category_id = ?";
			$stmt = $this->conn->prepare($sqlQuery);
			$stmt->bind_param("i", $this->category_id); // Use $this->category_id
			$stmt->execute();
			$result = $stmt->get_result();
